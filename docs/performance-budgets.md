...
|---|---|---|---|---|---|---|---|---|---|---|
| `blocks-list-page-1` | p50 33.8 / p95 37.9 / p99 42.0 ms, 5.00 statements, 0 temp | p95 200 ms / p99 400 ms, ≤4 statements, 0 temp bytes | `generated` | target | cold | explorer | UR-1 | `pnpm bench --only blocks-list-page-1 --mode smoke` | FAIL [`7b1731d5`](performance/baselines/target-7b1731d5.json) | Cursor pagination plus a hard depth cap |
| `blocks-list-page-deep` | p50 35.4 / p95 40.1 / p99 45.0 ms, 5.00 statements, 0 temp | p95 300 ms / p99 600 ms, ≤4 statements, 0 temp bytes | `generated` | target | cold | explorer | UR-1 | `pnpm bench --only blocks-list-page-deep --mode smoke` | FAIL [`7b1731d5`](performance/baselines/target-7b1731d5.json) | Depth cap with a documented error past it |
| `transactions-list-page-1` | p50 54.3 / p95 59.1 / p99 65.6 ms, 5.00 statements | p95 200 ms / p99 400 ms, ≤4 statements | `generated` | target | cold | explorer | UR-1 | `pnpm bench --only transactions-list-page-1 --mode smoke` | FAIL [`7b1731d5`](performance/baselines/target-7b1731d5.json) | As above |
| `transactions-list-page-deep` | p50 58.2 / p95 71.4 / p99 90.8 ms, 5.00 statements, 2.9 MB temp | p95 300 ms / p99 600 ms, ≤4 statements | `generated` | target | cold | explorer | UR-1 | `pnpm bench --only transactions-list-page-deep --mode smoke` | FAIL [`7b1731d5`](performance/baselines/target-7b1731d5.json) | As above |
| `block-detail` | p50 23.9 / p95 32.3 / p99 44.8 ms, 19.00 statements, 7.3 KB wire, 30.6 KB identity | p95 250 ms / p99 500 ms, ≤8 statements, ≤128 KB wire, ≤512 KB identity | `generated` | target | unique-key | explorer | none | `pnpm bench --only block-detail --mode smoke` | FAIL [`7b1731d5`](performance/baselines/target-7b1731d5.json) | **Bound the decoded transaction rows first**, see below |
| `transaction-detail` | p50 16.1 / p95 23.0 / p99 29.2 ms, 16.00 statements, 5.0 KB wire, 4.3 KB identity | p95 250 ms / p99 500 ms, ≤8 statements, ≤192 KB wire, ≤768 KB identity | `generated` | target | unique-key | explorer | none | `pnpm bench --only transaction-detail --mode smoke` | FAIL [`7b1731d5`](performance/baselines/target-7b1731d5.json) | Lower `MAX_INLINE_CBOR_BYTES` from 64 KB |
| `search-prefix` | p50 20.5 / p95 23.7 / p99 25.6 ms, 5,384 shared blocks | p95 400 ms / p99 800 ms, ≤200k shared blocks (`hit + read`) | `real+extended` | target | unique-key | explorer | none | `pnpm bench --only search-prefix --mode smoke` | PASS [`7b1731d5`](performance/baselines/target-7b1731d5.json) | Raise `MIN_PREFIX`; or an expression index, which is upstream-owned |
| `metrics` | p50 57.5 / p95 65.9 / p99 76.2 ms, 12.00 statements | p95 500 ms / p99 1,000 ms, ≤9 statements | `generated` | target | cold | explorer | none | `pnpm bench --only metrics --mode smoke` | FAIL [`7b1731d5`](performance/baselines/target-7b1731d5.json) | Split the panel into independent snapshots |
| `metrics-cached` | p50 0.6 / p95 1.1 / p99 1.7 ms, 0 statements | p95 20 ms / p99 40 ms | `generated` | target | warm | explorer | none | `pnpm bench --only metrics-cached --mode smoke` | PASS [`7b1731d5`](performance/baselines/target-7b1731d5.json) | n/a, this measures the cache on purpose |
| `asset-roster` | p50 211.2 / p95 240.4 / p99 260.5 ms, 5.00 statements | p95 800 ms / p99 1,600 ms, ≤2 statements | `generated` | target | cold | explorer | none | `pnpm bench --only asset-roster --mode smoke` | FAIL [`7b1731d5`](performance/baselines/target-7b1731d5.json) | Lower `SCAN_LIMIT` and report reduced coverage |
| `address-history` | p50 206.8 / p95 231.7 / p99 283.3 ms, 6.00 statements | p95 250 ms / p99 500 ms, ≤6 statements | `real+extended` | target | unique-key | explorer | none | `pnpm bench --only address-history --mode smoke` | PASS [`7b1731d5`](performance/baselines/target-7b1731d5.json) | Cursor pagination |
| `overview-aggregate` | not measured | p95 600 ms / p99 1,200 ms, ≤256 KB wire, ≤1 MB identity | `generated` | target | cold | explorer | none | `pnpm bench --only overview-aggregate --mode smoke` | PENDING BASELINE (frontend origin, excluded from the backend sweep) | One backend aggregate route |
| `blocks-list-saturation` | p50 636.9 / p95 699.7 / p99 726.7 ms, 50.0 req/s, 0 errors, 0 timeouts | p95 1,500 ms / p99 3,000 ms at concurrency 32, **≥20 req/s**, timeouts ≤0.1%, errors ≤1% | `generated` | target | cold | explorer | UR-1 | `pnpm bench --only blocks-list-saturation --mode smoke` | PASS [`7b1731d5`](performance/baselines/target-7b1731d5.json) | Raise pool `max`, or shed load at the edge |
...
|---|---|---|---|---|---|---|
| Backend peak RSS, `target` profile | not measured | ≤512 MB | explorer | none | `pnpm bench --profile target --mode baseline` (`peakRssBytes`) | PENDING BASELINE |
