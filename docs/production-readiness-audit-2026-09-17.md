...
| L7 hydration warning | **Open, not reproduced** | 24 further loads against a development server at 390 px, across both OS colour schemes and three stored-theme states, on four routes including the block page, with the stored theme confirmed applied. Zero warnings, on top of the 40 production loads already recorded. The original sighting was under memory pressure, about 480 MB available; that condition was not recreated, so absence here is not evidence it is fixed. 2026-09-19: ten further loads through the development-console gate, which now resolves a real block hash and loads every route at 390 and 1280. It previously loaded neither a record page nor a narrow viewport, while its own comment claimed a record page was covered. All ten clean; the memory-pressure condition is still not recreated |
...
| Change anything for backend memory that the profiler has not measured first | The one change made so far, narrowing three import sites off the `@lucid-evolution/lucid` barrel, followed a measurement that named the barrel and was re-measured after: floor 270.1 to 221.5 MB, peak 384.8 to 369.8 MB. Response size still does not imply retained heap |
...
| **Developer experience** | Every substantial JavaScript tool is either statically checked or listed, with its error count, in the config that will take it next | `backend/tsconfig.scripts.json` and `frontend-new/app/tsconfig.scripts.json`, both in their gates and in CI | **PARTIAL.** `snapshot.mjs` (978 lines) and `measure.mjs` (247) are checked and clean. The queue is named in the backend config with counts: `lib/checks.mjs` 48, `lib/compose.mjs` 22, `coverage-lint.mjs` 21, `setup.mjs` 20, and the browser fixtures at 53 for `server.mjs` and 44 for `data.mjs` |
