...
import { fetchDepositsPage } from "../api/deposits";
import type { DepositRow, DepositStatus } from "../api/deposits";
...
const STATUS_STYLES: Record<DepositStatus, string> = {
  ...
  consumed: "bg-emerald-400/10 text-emerald-300 ring-1 ring-emerald-400/30",
};
...
export default function DepositsPage() {
  ...
  return (
    <PageShell>
      <SectionHeader
        title="Deposits"
        subtitle="Browse L1 to L2 deposits and their projection status."
      />
      ...
      <div className="mt-8 grid gap-6">
        <GlassCard className="p-6">
          ...
          <div className="mt-4 overflow-x-auto rounded-2xl border border-white/10">
            <div className="grid min-w-[900px] grid-cols-[110px_1.3fr_1.3fr_160px_180px_140px] gap-0 bg-white/5 text-xs uppercase tracking-[0.22em] text-slate-400">
              <div className="px-4 py-3">Status</div>
              <div className="px-4 py-3">L1 Tx Hash</div>
              <div className="px-4 py-3">L2 Address</div>
            ...
            <div className="min-w-[900px] divide-y divide-white/10">
              {rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <div
                    key={row.event_id}
                    className="grid grid-cols-[110px_1.3fr_1.3fr_160px_180px_140px] gap-0 bg-slate-950/40"
                  >
                    <div className="px-4 py-4">
                      <StatusBadge status={row.status} />
                    </div>
                    <div className="px-4 py-4 text-xs text-slate-200 font-mono">
                      <span title={row.deposit_l1_tx_hash}>
                        {formatHash(row.deposit_l1_tx_hash)}
                      </span>
                    </div>
                    <div className="px-4 py-4 text-xs text-slate-200 font-mono">
                      <Link
                        to={`/address/${row.ledger_address}`}
                        title={row.ledger_address}
                        className="hover:text-cyan-200"
                      >
                        {formatHash(row.ledger_address, 12, 8)}
                      </Link>
                    </div>
                    <div className="px-4 py-4 text-xs text-slate-200 font-mono">
                      {row.value ? formatValueView(row.value) : "—"}
                    </div>
