...
import {
  ...
  parseAddressTxs,
  parseTxInfo,
  type KoiosAccountUpdate,
  type KoiosAddressTx,
  type KoiosTxInfo,
} from "./koios-schemas";

/** The payload shapes live next door; re-exported so this module's public
 * surface is unchanged by the split. */
export * from "./koios-schemas";
 ...
 *
 * The indexer's cursors used to record the highest block in which a
 * Midgard-related transaction was found, seeded from the previous cursor, so an
 * empty scan left them where they were. A quiet deployment therefore sat
 * thousands of blocks below the tip while being perfectly synchronised, and a
 * stopped writer looked identical. Comparing that number to the tip was not
 * merely insufficient, it compared two different quantities.
 *
 * Sampling the tip at the start of a pass makes the cursor a coverage
 * watermark: every source scanned from the shared floor up to at least here, so
 * advancing to this height once they all completed says what the index actually
 * covers. Anything that arrived while the pass ran is below the next pass's
 * lookback and is picked up then.
 */
export async function fetchTip(): Promise<KoiosTip> {
  const rows = z.array(tipSchema).parse(await get("/tip"));
  const row = rows[0];
  if (!row) throw new Error("Koios /tip returned no rows");
  return { blockHeight: row.block_height, blockTime: row.block_time };
}
